...
export function CommunityProfile({ profileId, profile, me, balance, pins, following, busy, login, onProfile, onEdit, onLogout, onFollow, onMap, onPinEdit, onPinDelete }) {
  ...
  return <main className="profile-page-x"><section className="profile-head"><img className="profile-avatar" src={profile.profileImage || profileImage()} alt={`${profile.username} 프로필`}/><h1>{profile.username}</h1><p>{profile.bio || '내 주변에서 코인 이야기를 나누세요.'}</p><div className="profile-social"><button onClick={() => setView('팔로워')}><b>{graph.followers.length}</b> 팔로워</button><button onClick={() => setView('팔로잉')}><b>{graph.following.length}</b> 팔로잉</button></div></section>
    ...
    <h2 className="profile-feed-title">{own ? '내 Feed' : `${profile.username}님의 Feed`}</h2><Segments items={['작성 피드','리포스트']} value={feed} onChange={setFeed}/><div className="feed">{!loaded ? <p role="status">피드를 불러오는 중…</p> : shown.length ? shown.map(post => <CommunityPost key={post.id} post={post} rank={ranks.get(post.coin)} onProfile={onProfile} {...actions}/>) : <Empty text={feed === '작성 피드' ? '아직 작성한 피드가 없습니다' : '아직 리포스트가 없습니다'}/>}</div>
